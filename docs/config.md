...
| `pause_after_nudge` | Pause after the wake so Connect registers the device without leaving audio playing. Playback then starts only from an explicit CLI/TUI play command. Set false to hear the automatically started playback. | `true` |
