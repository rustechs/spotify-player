...
| Option | Description | Default |
| --- | --- | --- |
| `enable` | Run wake during playback init when `preferred_device` is missing or idle/paused (or when no devices are listed if unset); also required for `spotify_player wake-desktop`. When Connect has no current playback, the TUI uses MPRIS metadata for the playback window. When Connect lists the preferred desktop client at 0% volume, MPRIS volume is shown instead. | `false` |
