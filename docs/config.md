...
Options in the `[desktop_spotify]` section launch and/or MPRIS-nudge the official desktop client when `preferred_device` is missing from Connect, or when it is listed but not actively playing (typical paused tray/autostart case). If the official client is already playing locally (MPRIS `Playing`), wake is skipped so that audio is not paused or stolen. If MPRIS is already Playing and Connect lists `preferred_device`, startup still transfers to that device with keep-playing and never to another speaker. If another speaker is already playing while preferred is listed, wake is skipped so audio is not stolen. If `preferred_device` is unset, wake only when no devices are listed. Common with `enable_streaming = "Never"` + `preferred_device`:
