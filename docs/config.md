...
| `playback_refresh_duration_in_ms` | Interval (ms) between playback refreshes. Set to `0` to refresh only on events or commands.             | `0`                                                                    |
...
- Setting a positive `playback_refresh_duration_in_ms` increases API usage and may trigger rate limits. By default it is `0` (refresh playback only on events or commands).
