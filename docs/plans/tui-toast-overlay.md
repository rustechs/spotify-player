...
**ADR(s):** none — this repository has no `docs/decision/` ADR process; durable choices are the Interview Output recorded in the planning session (purpose, overlay vs `PopupState`, FIFO, timed errors, placement, triggers, `esc`, config, overflow), plus the 2026-08-16 override: three stacked cards + `4+` marker; errors share `toast_success_timeout_secs`.
...
In the interactive TUI, a mutating action (queue/library/playlist/like/follow/create playlist, copy link, clipboard-open) shows a short non-modal toast: success and error toasts auto-dismiss after `toast_success_timeout_secs` (default 3); `esc` dismisses the current toast early when no popup is open. Multiple toasts are FIFO with cap 10 (drop newest, `tracing::warn!`); UI shows up to three stacked cards and a `4+` marker for additional queued messages. Desktop track-change notify is unchanged. Daemon/CLI never render toasts.
...
|---|---|---|---|---|---|
| Leftover rect after `render_playback_window` is the only safe toast clip | Overlaying `frame.area()` paints on cover art (#498) | Read `split_rect_for_playback_window`; render toast with that rect, not full frame | Unit-test a pure `toast_stack_areas(content, heights, overflow)` that stays inside `content` (x+width ≤ content.right, y+height ≤ content.bottom) | Manual TUI with `image` feature | Pass: helper never returns a rect outside content; fail: any overlap with playback outer rect |
...
| Success timeout can use `Instant` + existing UI refresh (`app_refresh_duration_in_ms`, default 32) | Need a dedicated timer thread | `ToastQueue::expire_due(now)` called from `ui::run` **before** `terminal.draw` | Unit test: success and error toasts with `expires_at` in the past are popped | — | Pass: fake clock tests; fail: toasts never expire |
...
| Cap-10 drop-newest does not drop the **visible** toast | User loses the error on screen | `push` refuses insert when `len() >= 10` without touching index 0 | Unit test: 10 queued, 11th dropped, current still first | — | Pass: current unchanged; warn path invoked (test hook or count) |
...
| UI-loop expiry | Success and error `expires_at` values pop; 32ms UI poll is enough to notice | `toast_queue_expires_successes_and_errors` — unit. `AppConfig` default `app_refresh_duration_in_ms: 32`. `ui::run` calls `expire_due` before draw. | Supported | Fake `Instant` in unit tests, not the real draw loop. 32ms vs 3s timeout is arithmetic, not measured in TUI. | High | None |
...
**Round summary:** Isolated `ToastQueue`, `toast_stack_areas`, `close_popup_or_dismiss_toast`, and `ClientRequest::is_toastable` are proven by unit tests and clippy exhaustive match. Cover-art overlap is still a human gate. `is_toastable` lives on `ClientRequest` to avoid a `state`↔`client` import cycle.
...
**Plan changes implied:** U1 is promotion of these types (do not rewrite). U4 uses `ClientRequest::is_toastable`. `toast_stack_areas` stacks up to three cards with a `4+` marker above the stack.
...
| U1 | `ToastQueue`: kind, message, `expires_at: Instant`, FIFO, visible stack, cap 10 drop-newest, `expire_due` | Interview q4–q6, q13 | `spotify_player/src/state/ui/toast.rs`, export from `state/ui/mod.rs` | P0 types | `toast_queue_drops_newest_at_cap` does not compile / fail | `cargo test -p spotify_player toast_queue` | S |
...
| U3 | Wire `UIState.toasts`; call `expire_due(Instant::now())` in `ui::run` before draw; render overlay lower-right of the leftover content rect (the `rect` after playback split, not `frame.area()`) with up to three stacked cards and a `4+` overflow marker | Interview q3, q5, q8; 2026-08-16 stack override | `state/ui/mod.rs`, `ui/mod.rs` `run` + `render_application`, `ui/toast.rs` | U1, U2 | `toast_stack_areas_stay_inside_content` fails until helper exists | unit tests + lint | M |
...
> **Phase N status — implementation complete.** U2–U6 plus next/prev toastable are wired. Remaining: human visual gates (stack/`4+`, cover art, theme) and merge.
...
|---|---|---|---|---|
| FIFO + three visible cards | unit `state/ui/toast.rs` | `cargo test toast_queue_visible_items_are_limited_to_three -- --exact` fail | same | enqueue 4, `visible` is first three |
| Success and error expire | unit | `cargo test toast_queue_expires_successes_and_errors` fail | same | fake `now` past `expires_at` |
...
| Toast rects inside content | unit `state/ui/toast.rs` | `cargo test toast_stack_areas_stay_inside_content` fail | same | several content sizes including tiny |
...
| Mutation error toast | same with forced API failure | developer TUI | Manual: error toast, wait 3s, gone |
| Stack + overflow | four rapid mutations | developer TUI | Three cards + `4+` |
...
| Tiny terminal | content smaller than toast | clamp/skip draw; no panic | unit | local | `toast_stack_areas` min size |
| Popup + error toast | ActionList open | esc closes popup (list handler); toast remains | unit + human | local | helper + TUI |
| Search / PlaylistCreate + error toast | those popups do not handle `ClosePopup` | global esc clears popup via `close_popup_or_dismiss_toast`; toast remains | unit | local | Search still-open case |
...
|---|---|---|---|---|
| Stacked cards + `4+` | no screenshot harness | Add four items quickly; confirm three bodies + `4+` | Operator note on PR | revert overlay |
...
| FIFO + three stacked cards + `4+` | U1, U3, human stack | tests + human gate |
| Toast timeout | U1, U2 | expiry tests for success and error toasts |
| Content lower-right; not on playback | U3, P0 | `toast_stack_areas` |
