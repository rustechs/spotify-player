...
| U1 | `ToastQueue`: kind, message, `expires_at`, FIFO, visible stack, cap 10 drop-newest, `expire_due` | Interview q4–q6, q13 | `spotify_player/src/state/ui/toast.rs`, export from `state/ui/mod.rs` | P0 types | `toast_queue_drops_newest_at_cap` does not compile / fail | `cargo test -p spotify_player toast_queue` | S |
